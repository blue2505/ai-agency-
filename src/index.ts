...
async function medSpaTTS(text: string): Promise<string> {
  ...
  if (!resp.ok) throw new Error(`ElevenLabs Sofia ${resp.status}`);
  fs.writeFileSync(abs, Buffer.from(await resp.arrayBuffer()));
  return rel;
}
...
- Maximum two sentences unless listing services

Hours: ${MEDSPA_HOURS}
Location: ${MEDSPA_SERVICE_AREAS}

Services and pricing:
- Botox: twelve dollars per unit, average two fifty to four hundred
...
- Chemical Peel: one forty nine to two ninety nine
...
Rules:
- If asked what services we offer, list them all with prices naturally
- If asked about a specific service, explain it and give the price
- Never treat a question as a booking answer
- Answer questions first then return to booking gently
- Never make medical claims
- Recommend consultation for specific concerns
- NEVER ask the same question twice
- Sound human every single time

Booking flow - collect naturally:
1. Name
2. Service interested in
3. Preferred day and time
4. Confirm warmly before finalizing`.trim();
}
...
async function handleMedSpaTurn(session: MedSpaSession, userSpeech: string): Promise<string> {
  if (!openai) return "I can help you book an appointment or answer questions. What can I help you with?";
...
Phone: ${session.callerPhone}`,
          ...
          }).then(e => app.log.info({ id: e.data.id }, "Med spa booked"))
...
Phone: ${session.callerPhone}`,
      ...
      const firstName = session.booking.name?.split(" ")[0] || "there";
      const reply = `You are all set ${firstName}! Your ${session.booking.service} appointment is confirmed for ${session.booking.time}. We look forward to seeing you at ${MEDSPA_COMPANY_NAME}!`;
      session.history.push({ role: "assistant", content: reply });
      return reply;
    }
    if (["no","wrong","change","different"].some(w => t.includes(w))) {
      ...
      session.booking = {};
      const reply = "No problem at all! Let me start over. What is your name?";
      session.history.push({ role: "assistant", content: reply });
      return reply;
    }
  }
...
app.post("/medspa-intake", async (req: any, reply: any) => {
  ...
  try {
    ...
    if (!speech) {
      ...
      await medSpaGather(twiml, prompts[Math.min(session.silenceCount - 1, prompts.length - 1)]);
      reply.type("text/xml");
      return reply.send(twiml.toString());
    }
    ...
    await medSpaGather(twiml, "I am sorry, I had a little trouble there. Could you say that again?");
    reply.type("text/xml");
    return reply.send(twiml.toString());
  }
});
