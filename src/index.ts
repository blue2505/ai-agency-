...
function looksLikeFaqQuestion(text: string) {
  const t = normalizeText(text);
  return (
    ...
    t.includes("weekend") ||
    t.includes("maintenance plan") ||
    t.includes("membership") ||
    t.includes("do you work on") ||
    t.includes("brands") ||
    t.includes("install") ||
...
function getSimpleFaqReply(text: string): string | null {
  ...
  if (t.includes("warranty")) {
    return "Warranty coverage can vary depending on the equipment and the service performed, but we can definitely go over that with you once we know the job details.";
  }
...
async function addPromptAndGather(
  ...
  const usePremiumVoice =
    ...
    !lower.includes("say that one more time") &&
    !lower.includes("say the email") &&
  ...
  try {
    ...
    if (!usePremiumVoice) {
      throw new Error("Using Twilio fallback for low-value prompt");
    }
...
- If the caller asks a broad pricing question, summarize briefly or ask which service they mean
...
async function answerQuestionDuringBooking(text: string) {
  if (looksLikePricingQuestion(text)) {
    ...
    if (isBroadPricingQuestion(text)) return priceMenu();
...
console.log("VOICE BODY:", req.body);
  ...
  try {
    ...
    if (looksLikeBookingIntent(speech)) {
      ...
      await addPromptAndGather(
        twiml,
        "Absolutely. I can help with that. What name should I put the appointment under?"
      );
      reply.type("text/xml");
      return reply.send(twiml.toString());
    }
    ...
    if (looksLikePricingQuestion(speech)) {
      ...
      if (isBroadPricingQuestion(speech)) {
        await addPromptAndGather(
          twiml,
          "I can definitely help with pricing. Which service are you asking about, like a diagnostic, tune-up, thermostat install, drain clearing, or something else?"
        );
        reply.type("text/xml");
        return reply.send(twiml.toString());
      }
    ...
    if (looksLikeUrgentRepair(speech)) {
      ...
      await addPromptAndGather(
        twiml,
        "I'm sorry you're dealing with that. We can definitely help with that. Would you like me to get you scheduled?"
      );
      reply.type("text/xml");
      return reply.send(twiml.toString());
    }
